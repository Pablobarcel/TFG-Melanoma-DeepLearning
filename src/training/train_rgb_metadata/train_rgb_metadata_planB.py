# src/training/train_cnn_rgb_metadata/train_rgb_metadata_planB.py
...
def train_rgb_meta_planb(batch_size=32, accumulation_steps=4, num_epochs=10):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"\n🚀 [RGB + Meta Plan B] Fase 1: Linear Probing | Modality Dropout Activado")

    train_ds = RGBMetadataDatasetPlanB("data/Splitted_data/experiment_200k_3classes/train.csv", get_train_transforms(), is_train=True, dropout_prob=0.3)
    val_ds = RGBMetadataDatasetPlanB("data/Splitted_data/experiment_200k_3classes/val.csv", get_eval_transforms(), is_train=False)
    ...
    logger = ExperimentLogger(experiment_name="fusion/rgb_meta_planB_fase1", config={"lr": lr_head_fase1, "modality_dropout": "30%"})
    best_auc = 0.0
    ...
    logger_fase2 = ExperimentLogger(
        experiment_name="fusion/rgb_meta_planB_fase2", 
        config={
            "lr_heads": base_lr, 
