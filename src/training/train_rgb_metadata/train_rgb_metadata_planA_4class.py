# src/training/train_cnn_rgb_metadata/train_rgb_metadata_planA_4class.py
...
from src.data.dataset_rgb_metadata_planA_4class import RGBMetadataDatasetPlanA4Class
from src.models.cnn_rgb_metadata.model_rgb_metadata_planA_4class import RGBMetadataModelPlanA4Class
...
def run_epoch(model, dataloader, criterion_A, criterion_B, optimizer, device, is_train=True, accumulation_steps=4):
    ...
    with torch.set_grad_enabled(is_train):
        for i, (img_rgb, feats, yA, yB) in enumerate(dataloader):
            ...
            loss = loss_A + loss_B # Normal en 4 clases
...
def train_rgb_meta_planA_4class(batch_size=32, accumulation_steps=4, num_epochs=10):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"\n🚀 [RGB + Meta Plan A - 4 CLASES] Fase 1: Linear Probing")

    train_ds = RGBMetadataDatasetPlanA4Class("data/Splitted_data/Final_dataset_4class_200k/train.csv", get_train_transforms())
    val_ds = RGBMetadataDatasetPlanA4Class("data/Splitted_data/Final_dataset_4class_200k/val.csv", get_eval_transforms())
    
    train_loader = DataLoader(train_ds, batch_size=batch_size, shuffle=True, num_workers=4)
    val_loader = DataLoader(val_ds, batch_size=batch_size, shuffle=False, num_workers=4)

    # 🚨 ACTUALIZAR CON LOS PATHS A LOS EXPERTOS (De 4 Clases)
    ...
    model = RGBMetadataModelPlanA4Class(
    ...
    logger = ExperimentLogger(experiment_name="fusion/rgb_meta_planA_4class_fase1", config={"lr": lr_head_fase1})
    best_auc = 0.0
    ...
    # 🚨 ARREGLADO: Usamos results_dir de la Fase 1
    ...
    logger_fase2 = ExperimentLogger(experiment_name="fusion/rgb_meta_planA_4class_fase2", config={"lr_heads": base_lr})
    best_auc_fase2 = 0.0
    patience_counter = 0
    num_epochs_fase2 = 15
