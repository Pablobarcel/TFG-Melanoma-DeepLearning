# src/training/train_cnn_rgb_metadata/train_rgb_metadata_planB_4class.py
...
from src.data.dataset_rgb_metadata_planB_4class import RGBMetadataDatasetPlanB4Class
from src.models.cnn_rgb_metadata.model_rgb_metadata_planB_4class import RGBMetadataModelPlanB4Class
...
def run_epoch(model, dataloader, criterion_A, criterion_B, optimizer, device, is_train=True, accumulation_steps=4):
    ...
    with torch.set_grad_enabled(is_train):
        for i, (img_rgb, feats, yA, yB) in enumerate(dataloader):
            ...
            # 🚨 ESTRUCTURA MULTICLASE NORMAL (Sin x2.0 en 4 clases)
            loss = loss_A + loss_B
...
def train_rgb_meta_planb_4class(batch_size=32, accumulation_steps=4, num_epochs=10):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"\n🚀 [RGB + Meta Plan B - 4 CLASES] Fase 1: Linear Probing | Modality Dropout")

    # 🚨 DATASET DE 200k (4 Clases) 🚨
    train_ds = RGBMetadataDatasetPlanB4Class("data/Splitted_data/Final_dataset_4class_200k/train.csv", get_train_transforms(), is_train=True, dropout_prob=0.3)
    val_ds = RGBMetadataDatasetPlanB4Class("data/Splitted_data/Final_dataset_4class_200k/val.csv", get_eval_transforms(), is_train=False)
    
    train_loader = DataLoader(train_ds, batch_size=batch_size, shuffle=True, num_workers=4, pin_memory=True)
    val_loader = DataLoader(val_ds, batch_size=batch_size, shuffle=False, num_workers=4, pin_memory=True)

    # 🚨 ACTUALIZAR ESTAS RUTAS (Deben apuntar a tus modelos expertos de 4 CLASES)
    rgb_pth = "experiments/cnn_vit_rgb/PONER_RUTA_RGB_4CLASSES/best_model.pth"
    meta_pth = "experiments/metadata_mlp/PONER_RUTA_METADATA_B_4CLASSES/best_model.pth"

    model = RGBMetadataModelPlanB4Class(
        rgb_weights_path=rgb_pth, 
        meta_weights_path=meta_pth,
        meta_input_dim=len(train_ds.feature_cols)
    ).to(device)

    # --- 🛡️ ESTRATEGIA: LINEAR PROBING ---
    print("🧊 Congelando ramas expertas (RGB y Metadatos Plan B)...")
    ...
    logger = ExperimentLogger(experiment_name="fusion/rgb_meta_planB_4class_fase1", config={"lr": lr_head_fase1, "modality_dropout": "30%"})
    best_auc = 0.0
    ...
    # ==========================================================
    print("✅ Fase 1 completada. Listo para hacer Fine-Tuning.")
    
    # 🚨 LA CARGA SEGURA CON results_dir
    ...
    logger_fase2 = ExperimentLogger(
        experiment_name="fusion/rgb_meta_planB_4class_fase2", 
        config={
            "lr_heads": base_lr, 
            "modality_dropout": "30% en Benignos",
            "fase": "Fine-Tuning End-to-End"
        }
    )
