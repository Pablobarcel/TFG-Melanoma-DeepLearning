# src/training/train_cnn_arp_metadata/train_arp_metadata_planB_3class.py
...
def run_epoch(model, dataloader, criterion_A, criterion_B, optimizer, device, is_train=True, accumulation_steps=4):
    ...
    with torch.set_grad_enabled(is_train):
        for i, (img_arp, feats, yA, yB) in enumerate(dataloader):
            ...
            # 🚨 Multiplicador estructural x2.0 para balancear gradientes en 3 clases
            ...
            predA = (probA >= 0.5).long() # 🚨 UMBRAL CLÍNICO NEUTRO 0.5
...
def train_arp_meta_planB_3class(batch_size=32, accumulation_steps=4, num_epochs=10):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"\n🚀 [ARP + Meta Plan B - 3 CLASES] Fase 1: Linear Probing | Modality Dropout Activado")

    # 🚨 DATASET: Modality Dropout configurado al 30% en Train
    train_ds = ARPMetadataDatasetPlanB3Class("data/Splitted_data/experiment_200k_3classes/train.csv", get_train_transforms_arp(), is_train=True, dropout_prob=0.3)
    val_ds = ARPMetadataDatasetPlanB3Class("data/Splitted_data/experiment_200k_3classes/val.csv", get_eval_transforms_arp(), is_train=False)
    
    train_loader = DataLoader(train_ds, batch_size=batch_size, shuffle=True, num_workers=4, pin_memory=True)
    val_loader = DataLoader(val_ds, batch_size=batch_size, shuffle=False, num_workers=4, pin_memory=True)

    # 🚨 ACTUALIZAR CON TUS RUTAS EXACTAS A LOS EXPERTOS (Deben ser los de 3 Clases)
    arp_pth = "experiments/cnn_arp/PONER_RUTA_ARP_3CLASSES/best_model.pth"
    meta_pth = "experiments/metadata_mlp/PONER_RUTA_METADATA_B_3CLASSES/best_model.pth"

    model = ARPMetadataModelPlanB3Class(
        arp_weights_path=arp_pth, 
        meta_weights_path=meta_pth,
        meta_input_dim=len(train_ds.feature_cols)
    ).to(device)

    # ==========================================================
    # 🛡️ FASE 1: LINEAR PROBING (Entrenar solo cabezas)
    # ==========================================================
    print("🧊 Congelando ramas expertas (ARP y Metadatos Plan B)...")
    ...
    logger = ExperimentLogger(experiment_name="fusion/arp_meta_planB_3class_fase1", config={"lr": lr_head_fase1, "modality_dropout": "30%"})
    best_auc = 0.0
    ...
    # ==========================================================
    # 🔓 FASE 2: FINE-TUNING GLOBAL (Differential LR Asimétrico)
    # ==========================================================
    print("✅ Fase 1 Completada. Iniciando Fase 2 (Descongelando todo)...")
    
    # 🚨 LA CARGA SEGURA DE PESOS DE LA FASE 1
    ...
    # 🧠 DIFFERENTIAL LR: Expertos Custom reciben Velocidad / 10
    ...
    logger_fase2 = ExperimentLogger(
        experiment_name="fusion/arp_meta_planB_3class_fase2", 
        config={
            "lr_heads": base_lr,
            "modality_dropout": "30%",
