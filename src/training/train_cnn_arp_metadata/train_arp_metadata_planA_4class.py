...
from src.evaluation.metrics import metrics_headA, metrics_headB # Métricas 4 clases
...
from src.data.dataset_arp_metadata_planA_4class import ARPMetadataDatasetPlanA4Class
from src.models.cnn_arp_metadata.model_arp_metadata_planA_4class import ARPMetadataModelPlanA4Class
...
def run_epoch(model, dataloader, criterion_A, criterion_B, optimizer, device, is_train=True, accumulation_steps=4):
    ...
    with torch.set_grad_enabled(is_train):
        for i, (img_arp, feats, yA, yB) in enumerate(dataloader):
            ...
            loss = loss_A + loss_B # Normal para 4 clases
...
def train_arp_meta_planA_4class(batch_size=32, accumulation_steps=4, num_epochs=10):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"\n🚀 [ARP + Meta Plan A - 4 CLASES] Fase 1: Linear Probing")

    train_ds = ARPMetadataDatasetPlanA4Class("data/Splitted_data/Final_dataset_4class_200k/train.csv", get_train_transforms_arp())
    val_ds = ARPMetadataDatasetPlanA4Class("data/Splitted_data/Final_dataset_4class_200k/val.csv", get_eval_transforms_arp())
    
    train_loader = DataLoader(train_ds, batch_size=batch_size, shuffle=True, num_workers=4, pin_memory=True)
    val_loader = DataLoader(val_ds, batch_size=batch_size, shuffle=False, num_workers=4, pin_memory=True)

    # 🚨 ACTUALIZAR CON LOS PATHS A TUS MODELOS DE 4 CLASES
    ...
    model = ARPMetadataModelPlanA4Class(
    ...
    logger = ExperimentLogger(experiment_name="fusion/arp_meta_planA_4class_fase1", config={"lr": lr_head_fase1})
    best_auc = 0.0
    ...
    logger_fase2 = ExperimentLogger(experiment_name="fusion/arp_meta_planA_4class_fase2", config={"lr_heads": base_lr})
