# src/training/train_cnn_arp_metadata/train_arp_metadata_planA_3class.py
...
from src.data.dataset_arp_metadata_planA_3class import ARPMetadataDatasetPlanA3Class
from src.models.cnn_arp_metadata.model_arp_metadata_planA_3class import ARPMetadataModelPlanA3Class
...
def run_epoch(model, dataloader, criterion_A, criterion_B, optimizer, device, is_train=True, accumulation_steps=4):
    ...
    with torch.set_grad_enabled(is_train):
        for i, (img_arp, feats, yA, yB) in enumerate(dataloader):
            ...
            loss = loss_A + (2.0 * loss_B) # Multiplicador estructural 3 clases
            ...
            predA = (probA >= 0.5).long() # UMBRAL CLÍNICO 0.5
...
def train_arp_meta_planA_3class(batch_size=32, accumulation_steps=4, num_epochs=10):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"\n🚀 [ARP + Meta Plan A - 3 CLASES] Fase 1: Linear Probing")

    train_ds = ARPMetadataDatasetPlanA3Class("data/Splitted_data/experiment_200k_3classes/train.csv", get_train_transforms_arp())
    val_ds = ARPMetadataDatasetPlanA3Class("data/Splitted_data/experiment_200k_3classes/val.csv", get_eval_transforms_arp())
    
    train_loader = DataLoader(train_ds, batch_size=batch_size, shuffle=True, num_workers=4, pin_memory=True)
    val_loader = DataLoader(val_ds, batch_size=batch_size, shuffle=False, num_workers=4, pin_memory=True)

    # 🚨 ACTUALIZAR CON LOS PATHS A TUS MODELOS DE 3 CLASES
    ...
    model = ARPMetadataModelPlanA3Class(
    ...
    print("🧊 Congelando ramas expertas (ARP y Metadatos)...")
    ...
    logger = ExperimentLogger(experiment_name="fusion/arp_meta_planA_3class_fase1", config={"lr": lr_head_fase1})
    best_auc = 0.0
    ...
    # 🧠 DIFERENCIAL LR: ARP y Metadatos son "expertos custom" -> reciben /10
    ...
    logger_fase2 = ExperimentLogger(experiment_name="fusion/arp_meta_planA_3class_fase2", config={"lr_heads": base_lr})
    best_auc_fase2 = 0.0
    patience_counter = 0
    num_epochs_fase2 = 15
