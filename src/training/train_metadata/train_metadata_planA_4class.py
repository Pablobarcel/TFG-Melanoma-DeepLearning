# src/training/train_metadata/train_metadata_planA_4class.py
...
from src.data.dataset_metadata_planA_4class import MetadataDatasetPlanA4Class
...
def train_plan_a_4class(
    ...
    num_epochs=20, 
    patience=5,
    experiment_path="metadata_mlp/experiment_0001_planA_4classes"
):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"\n🚀 [Metadata Plan A - 4 CLASES] Modelo: MLP Tabular | Clinical Mode")

    config = {
        "model": "MetadataMLP (Plan A 4 Clases)", 
        "lr": learning_rate, 
    ...
    model = MetadataMLPPlanA4Class(input_dim=input_dim).to(device)
