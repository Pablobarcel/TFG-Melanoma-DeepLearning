# src/training/train_metadata/train_metadata_planA.py
...
def train_plan_a(
    ...
    num_epochs=15, 
    patience=4,
    experiment_path="metadata_mlp/experiment_0001_planA_3classes"
):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"\n🚀 [Metadata Plan A - 3 CLASES] Modelo: MLP Tabular | Clinical Mode")

    config = {
        "model": "MetadataMLP (Plan A)", 
        "lr": learning_rate, 
    ...
    for epoch in range(num_epochs):
        ...
        logger.update_csv({"epoch": epoch + 1, "train_loss": train_loss, "val_loss": val_loss, "val_f1": val_metrics['headB']['macro_f1'], "lr": current_lr})
