...
def run_epoch(model, dataloader, criterion_A, criterion_B, optimizer, device, is_train=True, accumulation_steps=4):
    ...
    with torch.set_grad_enabled(is_train):
        for i, (img_rgb, img_arp, yA, yB) in enumerate(dataloader):
            ...
            loss = loss_A + loss_B # Normal 4 clases
...
def train_triple_model(batch_size=32, accumulation_steps=4, num_epochs=10):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"\n🚀 [RGB + ARP - 4 CLASES] Fase 1: Linear Probing")

    train_ds = HybridTripleDataset("data/Splitted_data/Final_dataset_4class_200k/train.csv", transforms_rgb=get_train_transforms(), transforms_arp=get_train_transforms_arp())
    val_ds = HybridTripleDataset("data/Splitted_data/Final_dataset_4class_200k/val.csv", transforms_rgb=get_eval_transforms(), transforms_arp=get_eval_transforms_arp())

    train_loader = DataLoader(train_ds, batch_size=batch_size, shuffle=True, num_workers=4, pin_memory=True)
    val_loader = DataLoader(val_ds, batch_size=batch_size, shuffle=False, num_workers=4, pin_memory=True)

    ruta_pesos_arp = "experiments/cnn_arp/PONER_RUTA_4C/best_model.pth" 
    ruta_pesos_rgb = "experiments/cnn_vit_rgb/PONER_RUTA_4C/best_model.pth"
    
    model = TripleHybridModel(num_classes_headB=4, pretrained_rgb=False, arp_pretrained_path=ruta_pesos_arp, rgb_pretrained_path=ruta_pesos_rgb).to(device)
    ...
    logger = ExperimentLogger(experiment_name="fusion/rgb_arp_4class_fase1", config={"lr": lr_head_fase1})
    best_auc = 0.0
    ...
    logger_fase2 = ExperimentLogger(experiment_name="fusion/rgb_arp_4class_fase2", config={"lr_heads": base_lr})
