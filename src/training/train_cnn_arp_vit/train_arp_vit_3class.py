...
from src.data.transforms import get_train_transforms, get_eval_transforms, get_train_transforms_arp, get_eval_transforms_arp
...
def run_epoch(model, dataloader, criterion_A, criterion_B, optimizer, device, is_train=True, accumulation_steps=4):
    ...
    with torch.set_grad_enabled(is_train):
        for i, (img_rgb, img_arp, yA, yB) in enumerate(dataloader):
            ...
            loss = loss_A + (2.0 * loss_B) # Multiplicador 3 Clases
...
def train_triple_model_3class(batch_size=32, accumulation_steps=4, num_epochs=10):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"\n🚀 [RGB + ARP - 3 CLASES] Fase 1: Linear Probing")

    train_ds = HybridTripleDataset3Class("data/Splitted_data/experiment_200k_3classes/train.csv", transforms_rgb=get_train_transforms(), transforms_arp=get_train_transforms_arp())
    val_ds = HybridTripleDataset3Class("data/Splitted_data/experiment_200k_3classes/val.csv", transforms_rgb=get_eval_transforms(), transforms_arp=get_eval_transforms_arp())

    train_loader = DataLoader(train_ds, batch_size=batch_size, shuffle=True, num_workers=4, pin_memory=True)
    val_loader = DataLoader(val_ds, batch_size=batch_size, shuffle=False, num_workers=4, pin_memory=True)

    ruta_pesos_arp = "experiments/cnn_arp/PONER_RUTA_3C/best_model.pth"
    ruta_pesos_rgb = "experiments/cnn_vit_rgb/PONER_RUTA_3C/best_model.pth"
    
    model = TripleHybridModel3Class(num_classes_headB=3, pretrained_rgb=False, arp_pretrained_path=ruta_pesos_arp, rgb_pretrained_path=ruta_pesos_rgb).to(device)
    ...
    logger = ExperimentLogger(experiment_name="fusion/rgb_arp_3class_fase1", config={"lr": lr_head_fase1})
    best_auc = 0.0
    ...
    for epoch in range(num_epochs):
        ...
        logger.log_full_report(train_metrics, epoch, phase="Train"); logger.log_full_report(val_metrics, epoch, phase="Val")
        best_auc = logger.save_checkpoint(model, val_metrics['headA']['auc'] if not np.isnan(val_metrics['headA']['auc']) else 0.0, best_auc)
    logger.close()

    # --- FASE 2: DIFFERENTIAL FINE-TUNING ---
    ...
    optimizer_fase2 = optim.AdamW([
        {'params': model.rgb_branch.parameters(), 'lr': base_lr / 100}, # Universal
        {'params': model.arp_branch.parameters(), 'lr': base_lr / 10},  # Custom
    ...
    logger_fase2 = ExperimentLogger(experiment_name="fusion/rgb_arp_3class_fase2", config={"lr_heads": base_lr})
    ...
    for epoch in range(15):
        ...
        print(f" Val Loss: {val_loss:.4f} | F1: {val_metrics['headB']['macro_f1']:.4f}")
        logger_fase2.log_full_report(train_metrics, epoch, phase="Train"); logger_fase2.log_full_report(val_metrics, epoch, phase="Val")
        ...
        if val_loss < best_val_loss_fase2:
            best_val_loss_fase2 = val_loss; patience_counter = 0
        else:
            patience_counter += 1
