# src/training/train_vit/train_hybrid_3class.py
...
def train_hybrid_model(
    learning_rate=9.16e-5,    # 🎯 Mejor LR de Optuna
    weight_decay=4.41e-3,     # 🎯 Mejor WD de Optuna
    batch_size=32,        
    accumulation_steps=4,     # 🎯 Validado por Optuna
    num_epochs=20, 
    patience=5,
    experiment_path="cnn_vit_rgb/experiment_final_3classes_tuned"
):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    effective_batch = batch_size * accumulation_steps
    print(f"\n🚀 [Hybrid RGB Run - 3 CLASSES NMSC] Batch Físico: {batch_size} | Matemático: {effective_batch}")
