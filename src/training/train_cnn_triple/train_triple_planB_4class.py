# src/training/train_cnn_triple/train_triple_planB_4class.py
...
# 🚨 Importamos las métricas estándar de 4 clases
...
def run_epoch(model, dataloader, criterion_A, criterion_B, optimizer, device, is_train=True, accumulation_steps=4):
    ...
    with torch.set_grad_enabled(is_train):
        for i, (img_rgb, img_arp, feats, yA, yB) in enumerate(dataloader):
            ...
            # 🚨 ESTRUCTURA 4 CLASES (Sin multiplicador)
            loss = loss_A + loss_B
...
def train_triple_planB_4class(batch_size=32, accumulation_steps=4, num_epochs=10):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"\n🚀 [JEFE FINAL: RGB + ARP + Meta Plan B | 4 CLASES] Fase 1: Linear Probing")

    # 🚨 DATASET DE 4 CLASES CON MODALITY DROPOUT
    train_ds = TripleDatasetPlanB4Class("data/Splitted_data/Final_dataset_4class_200k/train.csv", 
                                        get_train_transforms(), get_train_transforms_arp(), is_train=True, dropout_prob=0.3)
    val_ds = TripleDatasetPlanB4Class("data/Splitted_data/Final_dataset_4class_200k/val.csv", 
    ...
    logger = ExperimentLogger(experiment_name="fusion/triple_planB_4class_fase1", config={"lr": lr_head_fase1, "modality_dropout": "30%"})
    best_auc = 0.0
    ...
    logger_fase2 = ExperimentLogger(
        experiment_name="fusion/triple_planB_4class_fase2", 
        config={"lr_heads": base_lr, "modality_dropout": "30% en Benignos", "fase": "Fine-Tuning End-to-End"}
    )
