# src/training/train_cnn_triple/train_triple_planA_3class.py
...
def run_epoch(model, dataloader, criterion_A, criterion_B, optimizer, device, is_train=True, accumulation_steps=4):
    ...
    with torch.set_grad_enabled(is_train):
        for i, (img_rgb, img_arp, feats, yA, yB) in enumerate(dataloader):
            ...
            predA = (probA >= 0.5).long() # UMBRAL NEUTRO 0.5
...
def train_triple_planA_3class(batch_size=32, accumulation_steps=4, num_epochs=10):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"\n🚀 [JEFE FINAL: RGB + ARP + Meta Plan A | 3 CLASES] Fase 1: Linear Probing")

    train_ds = TripleDatasetPlanA3Class("data/Splitted_data/experiment_200k_3classes/train.csv", 
                                        get_train_transforms(), get_train_transforms_arp())
    val_ds = TripleDatasetPlanA3Class("data/Splitted_data/experiment_200k_3classes/val.csv", 
                                      get_eval_transforms(), get_eval_transforms_arp())
    ...
    logger = ExperimentLogger(experiment_name="fusion/triple_planA_3class_fase1", config={"lr": lr_head_fase1})
    best_auc = 0.0
    ...
    logger_fase2 = ExperimentLogger(experiment_name="fusion/triple_planA_3class_fase2", config={"lr_heads": base_lr})
