# src/training/train_cnn_triple/train_triple_planA_4class.py
...
# 🚨 Importamos las métricas estándar para 4 clases
...
def run_epoch(model, dataloader, criterion_A, criterion_B, optimizer, device, is_train=True, accumulation_steps=4):
    ...
    with torch.set_grad_enabled(is_train):
        for i, (img_rgb, img_arp, feats, yA, yB) in enumerate(dataloader):
            ...
            # 🚨 Loss normal para 4 clases (Sin multiplicador x2.0)
            loss = loss_A + loss_B
...
def train_triple_planA_4class(batch_size=32, accumulation_steps=4, num_epochs=10):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"\n🚀 [JEFE FINAL: RGB + ARP + Meta Plan A | 4 CLASES] Fase 1: Linear Probing")

    # 🚨 RUTAS AL DATASET DE 200k PARA 4 CLASES
    train_ds = TripleDatasetPlanA4Class("data/Splitted_data/Final_dataset_4class_200k/train.csv", 
                                        get_train_transforms(), get_train_transforms_arp())
    val_ds = TripleDatasetPlanA4Class("data/Splitted_data/Final_dataset_4class_200k/val.csv", 
                                      get_eval_transforms(), get_eval_transforms_arp())
    
    train_loader = DataLoader(train_ds, batch_size=batch_size, shuffle=True, num_workers=4, pin_memory=True)
    val_loader = DataLoader(val_ds, batch_size=batch_size, shuffle=False, num_workers=4, pin_memory=True)

    # 🚨 RUTAS A TUS 3 EXPERTOS BASE (De 4 Clases)
    ...
    meta_pth = "experiments/metadata_mlp/PONER_RUTA_METADATA_A_4CLASSES/best_model.pth"

    model = TripleModelPlanA4Class(
    ...
    # FocalLoss para 4 clases
    criterion_headB = FocalLoss(weight=compute_class_weights(train_ds.df, device), gamma=2.0)
    
    logger = ExperimentLogger(experiment_name="fusion/triple_planA_4class_fase1", config={"lr": lr_head_fase1})
    best_auc = 0.0
    ...
    logger_fase2 = ExperimentLogger(experiment_name="fusion/triple_planA_4class_fase2", config={"lr_heads": base_lr})
