# src/training/train_cnn_triple/train_triple_planB_3class.py
...
# Importamos las transformaciones correctas
...
def run_epoch(model, dataloader, criterion_A, criterion_B, optimizer, device, is_train=True, accumulation_steps=4):
    ...
    with torch.set_grad_enabled(is_train):
        for i, (img_rgb, img_arp, feats, yA, yB) in enumerate(dataloader):
            ...
            # 🚨 Multiplicador estructural x2.0 en 3 clases
            ...
            predA = (probA >= 0.5).long() # UMBRAL NEUTRO
...
def train_triple_planB_3class(batch_size=32, accumulation_steps=4, num_epochs=10):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"\n🚀 [JEFE FINAL: RGB + ARP + Meta Plan B | 3 CLASES] Fase 1: Linear Probing")

    # 🚨 DATASET: Modality Dropout al 30%
    train_ds = TripleDatasetPlanB3Class("data/Splitted_data/experiment_200k_3classes/train.csv", 
                                        get_train_transforms(), get_train_transforms_arp(), is_train=True, dropout_prob=0.3)
    val_ds = TripleDatasetPlanB3Class("data/Splitted_data/experiment_200k_3classes/val.csv", 
                                      get_eval_transforms(), get_eval_transforms_arp(), is_train=False)
    
    train_loader = DataLoader(train_ds, batch_size=batch_size, shuffle=True, num_workers=4, pin_memory=True)
    val_loader = DataLoader(val_ds, batch_size=batch_size, shuffle=False, num_workers=4, pin_memory=True)

    # 🚨 RUTAS A TUS 3 EXPERTOS BASE (3 Clases)
    ...
    # ==========================================================
    print("🧊 Congelando las 3 ramas expertas...")
    ...
    logger = ExperimentLogger(experiment_name="fusion/triple_planB_3class_fase1", config={"lr": lr_head_fase1})
    best_auc = 0.0
    ...
    # ==========================================================
    # 🔓 FASE 2: DIFFERENTIAL FINE-TUNING (A 3 Velocidades)
    # ==========================================================
    ...
    # 🧠 DIFERENCIAL LR SUPREMO
    optimizer_fase2 = optim.AdamW([
        {'params': model.rgb_branch.parameters(), 'lr': base_lr / 100}, # Universal (ImageNet)
        {'params': model.arp_branch.parameters(), 'lr': base_lr / 10},  # Custom (Tu CNN)
        {'params': model.meta_branch.parameters(), 'lr': base_lr / 10}, # Custom (Tu MLP)
    ...
    logger_fase2 = ExperimentLogger(experiment_name="fusion/triple_planB_3class_fase2", config={"lr_heads": base_lr})
    ...
    logger_fase2.close()
    print("✅ Entrenamiento TRIPLE finalizado con éxito.")

if __name__ == "__main__":
    train_triple_planB_3class()