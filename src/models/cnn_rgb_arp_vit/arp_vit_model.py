...
class TripleHybridModel(nn.Module):
    """
    Fusión Visual Doble: ResNet18 + ViT-Tiny (RGB) + CNN Custom (ARP).
    Configurado para 4 Clases.
    """
    def __init__(self, num_classes_headB=4, pretrained_rgb=True, arp_pretrained_path=None, rgb_pretrained_path=None):
        ...
        if arp_pretrained_path:
            print(f"🔄 Inyectando conocimiento previo en rama ARP desde: {arp_pretrained_path}")
        ...
        if rgb_pretrained_path:
            print(f"🔄 Inyectando conocimiento previo en rama RGB desde: {rgb_pretrained_path}")
        ...
        # Mejoras de estabilización aplicadas a 4 clases
